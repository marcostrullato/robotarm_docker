...
#Dockerfile varibles
ARG ROBOTARM_WS=/root/moveit_ws
...
RUN apt-get update;
RUN DEBIAN_FRONTEND=noninteractive apt-get -y install ros-melodic-catkin python-catkin-tools python-rosdep build-essential pkg-config screen ros-melodic-moveit ros-melodic-collada-parser ros-melodic-collada-urdf net-tools iputils-ping screen git
...
RUN /bin/bash -c "set -x; . /opt/ros/melodic/setup.bash; . ${ROBOTARM_WS}/devel/setup.bash; \
    python /usr/local/lib/python2.7/dist-packages/openravepy/_openravepy_/ikfast.py \
